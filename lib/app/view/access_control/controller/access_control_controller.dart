...
import 'package:al_wasyeah/app/view/access_control/model/context_model.dart';
import 'package:al_wasyeah/app/view/access_control/model/user_context_model.dart';
...
class AccessControlController extends GetxController {
  ...
  Future<void> getUsersByRole() async {
    ...
    var response = await ApiService.getData("${ApiConstants.accessControl}$isWitnessParam");

    if (response.statusCode == 200 || response.statusCode == 201) {
      var data = jsonEncode(response.body);
      usersList.value = accessControlUserModelFromJson(data);
    } else {
      ToastMessage.errorMessageShowToster("Failed to load users");
    }
  ...
  Future<void> fetchContextsData(String requestKey) async {
    if (allContexts.isEmpty) {
      var response = await ApiService.getData(ApiConstants.accessFeature);
      if (response.statusCode == 200 || response.statusCode == 201) {
        var data = jsonEncode(response.body);
    ...
    if (!userSelectedContexts.containsKey(requestKey)) {
      ...
      var userContextRes = await ApiService.getData("${ApiConstants.accessSelect}?requestKey=$requestKey&trace=false");
  ...
  Future<void> fetchContextsPanelData(String requestKey) async {
    ...
    try {
      var response = await ApiService.getData(ApiConstants.accessControlPanelData + "?requestKey=$requestKey");
  ...
  Future<void> saveAllChanges() async {
    ...
    try {
      for (String key in changedUserKeys) {
        ...
        var response = await ApiService.postData("${ApiConstants.addFeatureNomineeWitnessPoint}$isWitnessParam", payload);
      ...
      if (allSuccess) {
        ToastMessage.successMessageShowToster("All changes saved successfully");
      } else {
        ToastMessage.errorMessageShowToster("Some changes failed to save");
      }
    } catch (e) {
      ToastMessage.errorMessageShowToster("An error occurred while saving");
    } finally {
