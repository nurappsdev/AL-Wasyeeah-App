...
import 'package:al_wasyeah/app/models/notification/inapp_notification_model.dart';
...
class NotificationController extends GetxController with StateMixin<List<InAppNotificationModel>> {
  ...
  Future<void> getNotificationList() async {
    ...
    try {
      var response = await ApiClient.getData(ApiConstants.inappnotificationList);
  ...
  Future<void> approveOrDeclienNotification(requestKey, value) async {
    try {
      var response = await ApiClient.getData(ApiConstants.inappNOtificationAcceptRejectRequest + "?requestKey=$requestKey&value=$value");
