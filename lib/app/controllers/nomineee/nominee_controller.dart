...
import '../../../core/services/toast_message_helper.dart';
...
class NomineeController extends GetxController with GetSingleTickerProviderStateMixin {
  ...
  deleteNominee({required String requestKey}) async {
    ...
    var response = await ApiClient.getData("${ApiConstants.deleteNominee(requestKey)}");
    if (response.statusCode == 200 || response.statusCode == 201) {
      ...
      ToastMessageHelper.errorMessageShowToster(AppLocalizations.of(Get.context!)!.failed_to_delete_nominee);
    }
    try {} catch (e) {}
  }
  ...
  Future<void> addYourNominee({
    ...
    if (response.statusCode == 200 || response.statusCode == 201) {
      ...
      ToastMessageHelper.errorMessageShowToster(AppLocalizations.of(Get.context!)!.add_failed_try_again);
    }
  }
